...
# 각 클래스에 대한 편향
b = np.random.randn(10)  # (10,): 각 클래스에 대한 편향
learning_rate = 0.01
epochs = 1000
...
for epoch in range(epochs):
    # 5-1. 예측 계산
    z = X_train_std @ w + b  # (1437, 10)

    # 5-2. 소프트맥스 함수 적용
    exp_z = np.exp(z - np.max(z, axis=1, keepdims=True))  # 오버플로우 방지
    prediction = exp_z / np.sum(exp_z, axis=1, keepdims=True)  # (1437, 10)

    # 5-3. 원-핫 인코딩
    y_one_hot = np.eye(10)[y_train]  # (1437, 10)

    # 5-4. 손실 계산
    loss = -np.mean(np.sum(y_one_hot * np.log(prediction + 1e-15), axis=1))  # 작은 값 추가
